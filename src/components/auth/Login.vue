        ...
        <v-layout  align-center justify-center>
            <v-flex xs12 sm8 md4>
                <v-card class="elevation-12">
                    ...
                    <v-card-text>
                        <v-form ref="formAuth">
                            <v-text-field v-if="!hasRegister" v-model.trim="user.name" v-bind:rules="nameRules" prepend-icon="person" name="name" label="Nome Completo" type="Nome"/>
                    ...
                    <v-text-field v-if="!hasRegister" v-bind:rules="passwordConfirm" prepend-icon="lock" name="password_confirm" label="Confirme a Senha" id="password_confirm" type="password"/>
                        </v-form>
                    </v-card-text>
                    <v-card-actions>
                        <v-spacer/>
                        <v-btn v-if="hasRegister" v-on:click="login()" class="white--text" color="teal darken-1">Entrar</v-btn>
                        <v-btn v-else v-on:click="register()" class="white--text" color="teal darken-1">Cadastrar</v-btn>
                    </v-card-actions>
                    <v-toolbar dark color="teal darken-1">
                        <a style="color: inherit" v-show="hasRegister" v-on:click="changeRegister(false)">Não tem cadastro? <strong>Cadastre-se!</strong></a>
                        <a style="color: inherit" v-show="!hasRegister" v-on:click="changeRegister(true)">Já tem cadastro? <strong>Entre!</strong></a>
                    </v-toolbar>
                </v-card>
